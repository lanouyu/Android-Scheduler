					...
					int trace)
{
	...
	/******************************************************/
	/*change the default scheduler and prority 
		of all descendants of process zygote*/
	struct task_struct *ancestor;
	for (ancestor = p; ancestor->pid != 0; ancestor = ancestor->parent)
	{
		if (strcmp("main", ancestor->parent->comm) == 0)
		{
			struct sched_param param;
			param.sched_priority = (99 / 5) * (p->pid % 5) + 1;
			sched_setscheduler(p, SCHED_RR, &param);
			printk("change <%s> default scheduler and proirity successfully!\n", p->comm);
			break;
		}
	}
